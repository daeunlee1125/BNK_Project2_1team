...
최우수상 수상작, AI와 음성 비서를 결합한 사용자 중심의 외환 특화 뱅킹 서비스

---

## 1. 프로젝트 개요

기존의 복잡하고 파편화된 외환 서비스(환율 조회, 환전, 예금)를 하나의 유기적인 '흐름(Flow)'으로 통합하였습니다. 초보 사용자도 상품 추천과 환율 변동성 기상청, 음성 가입 가이드를 통해 어렵게 느껴지는 외환 상품을 쉽게 이해하고 가입할 수 있는 환경을 제공합니다.
...
* **Global Overlay UI**: Flutter의 전역 오버레이를 활용하여, 화면에 의존하지 않고 음성 대화를 끊김 없이 이어가며 가입을 진행할 수 있도록 구현했습니다.
